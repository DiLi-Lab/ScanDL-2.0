...
class TransformerNetModel(nn.Module):
    ...
    def __init__(
        ...
        one_noise_step,
        mask_padding,
        fix_dur_emb_dim: int=1000,  # 5871  # 1000 because I mapped all fix durs >= 1000 to 999 
        dropout=0,
        config=None,
        ...
        self.sn_sp_fix_dur_embedding = nn.Embedding(fix_dur_emb_dim, self.input_dims)
        self.fix_dur_head = nn.Linear(self.input_dims, fix_dur_emb_dim)
        with torch.no_grad():
            self.fix_dur_head.weight = self.sn_sp_fix_dur_embedding.weight
    ...
    def get_embeds(
        ...
        fix_dur_emb = self.sn_sp_fix_dur_embedding(sn_sp_fix_dur)
        return sn_sp_emb, pos_enc, sn_input_ids_emb, fix_dur_emb
    ...
    def get_logits_fix_dur(self, model_output):
        if self.logits_mode == 1:
            return self.fix_dur_head(model_output)
        elif self.logits_mode == 2:  # standard cosine similarity
            raise NotImplementedError('standard cosine similarity not yet implemented for sp model output.')
        else:
            raise NotImplementedError
    ...
    def forward(
        ...
        :param x: the noised fixation location and fixation duration embeddings
